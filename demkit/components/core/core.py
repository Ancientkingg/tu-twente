...
# Logging functions
	...
	def logMsg(self, msg):
		if self.enableMsg:
			...
			sys.stderr.write(t+" | \033[95m\033[94m[INF]\033[0m "+msg+"\n")
			sys.stderr.flush()
			...
			if self.writeMsg:
				...
				f = open(filename, 'a')
				f.write(t+" | \033[95m\033[94m[INF]\033[0m "+msg+"\n")
				f.close()

	def logWarning(self, msg):
		if self.enableWarning:
			t = self.timeObject(time.time()).astimezone(demCfg['timezone']).strftime("%X")
			sys.stderr.write(t + " | \033[95m\033[93m[WRN]\033[0m " + msg + "\n")
			sys.stderr.flush()
			...
			if self.writeWarning:
				...
				os.makedirs(os.path.dirname(demCfg['var']['log']), exist_ok=True)
				f = open(filename, 'a')
				f.write(t + " | \033[95m\033[93m[WRN]\033[0m " + msg + "\n")
				f.close()

	def logError(self, msg):
		if self.enableError:
			t = self.timeObject(time.time()).astimezone(demCfg['timezone']).strftime("%X")
			sys.stderr.write(t + " | \033[95m\033[91m[ERR]\033[0m " + msg + "\n")
			sys.stderr.flush()
			...
			if self.writeError:
				...
				os.makedirs(os.path.dirname(demCfg['var']['log']), exist_ok=True)
				f = open(filename, 'a')
				f.write(t + " | \033[95m\033[91m[ERR]\033[0m " + msg + "\n")
				f.close()
	...
	def logDebug(self, msg):
		if self.enableDebug:
			t = self.timeObject(time.time()).astimezone(demCfg['timezone']).strftime("%X")
			sys.stderr.write(t + " | \033[95m\033[92m[DBG]\033[0m " + msg + "\n")
			sys.stderr.flush()
			...
			if self.writeDebug:
				...
				os.makedirs(os.path.dirname(demCfg['var']['log']), exist_ok=True)
				f = open(filename, 'a')
				f.write(t + " | \033[95m\033[92m[DBG]\033[0m " + msg + "\n")
				f.close()
