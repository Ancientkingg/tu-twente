...
class InfluxDB:
	def __init__(self, host):
		...
		print(self.database, self.address, self.port)
		...
		self.autoCleanup = (
			True  # Cleanup text files when data is restored after a hiccup
		)
		self.errorFlag = False

		self.filepath = (
			demCfg["var"]["databasebackup"] + host.name + "/"
	...
	def writeData(self, force=False):
		if len(self.data) > self.maxBuffer or force:
			...
			if (
				self.activeThreads == 0
			):  # For now we limit this to one thread for writing
	...
	def writeToDatabase(self, data):
		...
		try:
			r = requests.post(
				self.address + ":" + self.port + "/write?db=" + self.database,
			...
			if r.status_code != 204:
				self.host.logWarning(
					"[InfluxDB] Could not write to database. Errorcode: "
					+ str(r.status_code)
					+ "\t\t"
					+ r.text
				)
			...
			self.host.logWarning(
				"[InfluxDB] Could not connect to database while writing, is it running?\n"
			)
			result = False
	...
	def clearDatabase(self):
		...
		try:
			...
			if r.status_code != 200:
				self.host.logWarning(
					"[InfluxDB] Could not drop the database. Check your InfluxDB. Errorcode: "
					+ str(r.status_code)
					+ "\t\t"
					+ r.text
				)
			...
			self.host.logWarning(
				"[InfluxDB] Could not connect to database while clearing, is it running?"
			)
	...
	def createDatabase(self):
		...
		try:
			...
			if r.status_code != 200:
				self.host.logWarning(
					"[InfluxDB] Could not create the database. make sure that it does not start with numbers and does not contain dashes. \nErrorcode: "
					+ str(r.status_code)
					+ "\t\t"
					+ r.text
				)
			...
			self.host.logWarning(
				"[InfluxDB] Could not connect to database while creating, is it running?"
			)
	...
	def writeTextFile(self):
		...
		try:
			...
			self.host.logWarning(
				"[InfluxDB] Could not find or create backup file: " + self.filename
			)
	...
	def cleanupTextFiles(self):
		if self.autoCleanup:
			try:
				...
				self.host.logWarning(
					"[InfluxDB] Could not find or create backup file folder: "
					+ self.filename
				)
