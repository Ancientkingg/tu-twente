...
from dev.temp.smartMeterDev import SmartMeterDev

# Host, required to control/coordinate the simulation itself
from hosts.restHost import RestHost			# Platform Host, in this case a simulation basis to perform simulations
from hosts.simHost import SimHost			# Simulation Host, a host that can simulate time
...
extendedLogging = True			# Extended logging adds more information (such as reactive power) but slows down the simulation
...
# Heating system
	...
	thermostat.temperatureSetpointHeating = 21.0 			# setpoint temperature for the zone
	thermostat.temperatureSetpointCooling = 23.0
	thermostat.temperatureMin = 21.0 	# min setpoint temperature for the zone (turn on heating overnight on cold days)
